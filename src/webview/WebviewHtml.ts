...
/**
 * Loads a template from media/ and fills in the values that can only be known at
 * runtime: the webview-scoped asset URIs, the CSP source and a per-load nonce.
 */
export function getWebviewHtml(
    webview: vscode.Webview,
    extensionUri: vscode.Uri,
    template: string = "webview.html",
    extra: Record<string, string> = {}
): string {
    const templateUri = vscode.Uri.joinPath(extensionUri, "media", template);
    const source = fs.readFileSync(templateUri.fsPath, "utf8");
